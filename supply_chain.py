...
trans_cost = {('Factory', 'Warehouse1'): 5,
              ('Factory', 'Warehouse2'): 7,
...
# Objective Function
model.cost = Objective(expr=sum(trans_cost[i, j] * model.x[i, j, p]
                                for i in nodes
                                for j in nodes
                                if (i,j) in trans_cost.keys()
                                for p in products) +
                            sum(holding_cost[i][p] * model.y[i, p]
                                for i in nodes
                                for p in products), sense=minimize)
