...
//   }
   ...
   if( paraNodePool || paraNodeToKeepCheckpointFileNodes || unprocessedParaNodes )
   {
      ...
      if( typeid(paraSolverPool) == typeid(BbParaSolverPoolForMinimization *) )     // paraSolverPool may not be always BbParaSolverPoolForMinimization
      {
   ...
   else
   {
      ...
      else
      {
         ...
         {
            if( ( interruptedFromControlTerminal || hardTimeLimitIsReached || memoryLimitIsReached ) && paraRacingSolverPool )
            {
         ...
         {
            if( !interruptedFromControlTerminal && !computationIsInterrupted && !hardTimeLimitIsReached && !memoryLimitIsReached )
            {
               if( typeid(paraSolverPool) == typeid(BbParaSolverPoolForMinimization *) )     // paraSolverPool may not be always BbParaSolverPoolForMinimization
               {
                  bbParaInitiator->setNumberOfNodesSolved( std::max(1ULL, dynamic_cast<BbParaSolverPoolForMinimization *>(paraSolverPool)->getTotalNodesSolved()) );
               ...
               if( bbParaInitiator->getGlobalBestIncumbentSolution() && allCompInfeasibleAfterSolution )
               {
                  lcts.globalBestDualBoundValue = bbParaInitiator->getGlobalBestIncumbentSolution()->getObjectiveFunctionValue();
                  if( paraNodePool->getNumOfNodes() > 0 )
                  {
            ...
            else
            {
               ...
               if( isRacingStage() && (!hardTimeLimitIsReached) && (!memoryLimitIsReached) )
               {
   ...
   if( paraSolverPool && osStatisticsFinalRun  && typeid(paraSolverPool) == typeid(BbParaSolverPoolForMinimization *) )     // paraSolverPool may not be always BbParaSolverPoolForMinimization
   {
      ...
      )
{
   ...
   else
   {
      ...
      {
         if( runningPhase == TerminationPhase || hardTimeLimitIsReached )
         {
            delete solverState;
            return 0;
         }
      ...
      )
{
   ...
   else
   {
      ...
      if( paraNode )
      {
         if( ( dynamic_cast<BbParaSolverPoolForMinimization *>(paraSolverPool)->getDualBoundValue(source) - paraNode->getDualBoundValue()) > 0.0 &&
...
//        << ", KEEP: nBoundChanges = " << dynamic_cast<BbParaDiffSubproblem *>(paraNode->getDiffSubproblem())->getNBoundChanges() << std::endl;
   ...
   assert( hardTimeLimitIsReached || memoryLimitIsReached || dynamic_cast<BbParaSolverPoolForMinimization *>(paraSolverPool)->getCurrentTask(source) != 0 );

   if( !hardTimeLimitIsReached && !memoryLimitIsReached )
   {
      ...
      )
{
   ...
   for(;;)
   {
	 ...
	 if( paraParams->getRealParamValue(TimeLimit) > 0.0 )
      ...
      case RampUpPhase:
      {
         if( selfSplitFinisedSolvers &&  (!hardTimeLimitIsReached) && (!memoryLimitIsReached) &&
...
{
   if( merging || initialNodesGenerated || hardTimeLimitIsReached || memoryLimitIsReached ||
