...
if 1==1:
    ...
    if 1==1:
        ...
        # Aux for demand response
        model.DR_rel_energy_missmatch = pyo.Var(model.Loads, initialize=0.0)
    ...
    if 1==1:
        ...
        if 1==1:
            def deltaP_energy_rule(model, l):
                return sum(model.deltaP[l,t] for t in model.time)/float(Pdem_max.loc[l,'max_kW']) == model.DR_rel_energy_missmatch[l]
