...
# Sets
products = {0:'Product1', 1:'Product2'} # Set of products
nodes = {0:'Factory', 1:'Warehouse1', 2:'Warehouse2', 3:'Retailer'}  # Set of nodes
...
# Decision Variables
model.x = Var(nodes, nodes, products, within=NonNegativeReals)  # Quantity shipped from each node to another for each product
model.y = Var(nodes, products, within=NonNegativeReals)  # Quantity held at each node for each product
...
for r in demand.keys():
    for p in products:
        model.demand_constraint.add(sum(model.x[i, r, p] for i in nodes if i != r) == demand[r][p])  # Demand constraint at each retailer

model.supply_capacity_constraint = ConstraintList()
for f in supply_capacity.keys():
    for p in products:
        model.supply_capacity_constraint.add(sum(model.x[f, j, p] for j in nodes if j != f) == supply_capacity[f][p])  # Supply capacity constraint at each factory

model.balance_constraint = ConstraintList()
for j in nodes:
    for p in products:
        model.balance_constraint.add(sum(model.x[i, j, p] for i in nodes if i != j) + model.y[j, p] ==
                                     sum(model.x[j, k, p] for k in nodes if k != j))  # Flow balance constraint at each node
