...
# -*- coding: utf-8 -*-
"""
    This script defines a simplified blending problem cast as a linear programming problem with multiple tanks and different time periods using pyomo
...
product_yield = {('Gasoline', 'Crude 1'): 0.8, ('Kerosine', 'Crude 1'): 0.05, ('Fuel Oil', 'Crude 1'): 0.10, ('Residual', 'Crude 1'): 0.05,
                 ('Gasoline', 'Crude 2'): 0.44, ('Kerosine', 'Crude 2'): 0.10, ('Fuel Oil', 'Crude 2'): 0.36, ('Residual', 'Crude 2'): 0.10}

# model formulation
model = pyo.ConcreteModel()

# variables
model.x = pyo.Var(((tank, feed, time) for tank in TANKS for feed in FEEDS for time in TIME), domain=pyo.NonNegativeReals)
model.y = pyo.Var(((tank, product, time) for tank in TANKS for product in PRODUCTS for time in TIME), domain=pyo.NonNegativeReals)

# objective (maximize profit= income- total costs)
income = sum(df_products.loc[p, 'Price'] * model.y[o, p, t] for o in TANKS for p in PRODUCTS for t in TIME)
raw_materials_cost = sum(
    (df_crudes.loc[f, 'Feed Costs'] + df_crudes.loc[f, 'Processing Cost']) * model.x[o, f, t] for o in TANKS for f in FEEDS for t in TIME)
supply_cost = sum(df_products.loc[p, 'supply_cost'] * model.y[o, p, t] for o in TANKS for p in PRODUCTS for t in TIME)
...
for o in TANKS:
    for p in PRODUCTS:
        for t in TIME:
            model.constraints.add(model.y[o, p, t] <= maximum_capacity[o, t, p])
            model.constraints.add(model.y[o, p, t] == sum(model.x[o, f, t] * product_yield[(p, f)] for f in FEEDS))
for o in TANKS:
    for f in FEEDS:
        for t in TIME:
            model.constraints.add(model.x[o, f, t] >= demand[o, t, f])
